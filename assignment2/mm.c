...
void *mm_realloc(void *oldbp, size_t size)
{
	...
	size_t size2=0, next_block=0, tmp_size=0,copy_size=0;
	//새 공간을 가리킬 포인터
	void *newbp;
...
static void place(void *bp, size_t asize) {
	...
	if ((csize - asize) >= BLOCKSIZE) {
		...
		bp = NEXT_BLKP(bp);
		put_head_foot(bp,csize - asize, 0);
